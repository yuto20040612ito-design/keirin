...
## 2. セットアップ確認スクリプトを走らせる（推奨）

以下を実行すれば、Python の確認・動作テスト・cron に貼る文字列の生成まで
まとめてやってくれる。何も壊さないので、まずこれを試すのが早い。
...
`[OK] 収集は動きました。` と出たら、そのあとに表示される cron 用のコマンドを
サーバーパネルに貼るだけ。**手順4まで飛ばしてよい。**

うまくいかなければ、出力をそのまま貼って相談すること。
以下は手動でやる場合の詳細。
